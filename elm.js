...
Elm.Main.make = function (_elm) {
   ...
   var walk = F2(function (keys,chad) {    return _U.update(chad,{vx: $Basics.toFloat(keys.x)});});
   var url = "https://raw.githubusercontent.com/Chadtech/jasper-birthday/master/";
   var sourcer = function (i) {    return A2($Basics._op["++"],url,A2($Basics._op["++"],"/",i));};
   var bound = 75;
   var movement = F2(function (dt,chad) {    var vx$ = _U.cmp(chad.x,bound) < 0 ? dt * chad.vx * 5 : 0;return _U.update(chad,{x: chad.x + vx$});});
   var update = F2(function (_p0,chad) {    var _p1 = _p0;return A2(movement,_p1._0,A2(walk,_p1._1,chad));});
   var view = F2(function (_p2,chad) {
      var _p3 = _p2;
      var _p6 = _p3._0;
      var _p5 = _p3._1;
      var jasperImage = $Graphics$Collage.toForm(A3($Graphics$Element.image,124,160,sourcer("jasper.png")));
      var chadImage = $Graphics$Collage.toForm(A3($Graphics$Element.image,136,168,sourcer("chad.png")));
      var bluer = $Graphics$Collage.filled(A3($Color.rgb,23,92,254));
      var nothing = bluer(A2($Graphics$Collage.rect,1,1));
      var chadSign = _U.cmp(chad.x,bound) < 0 ? $Graphics$Collage.toForm(A3($Graphics$Element.image,42,55,sourcer("chad-sign.png"))) : nothing;
      var jasperSign = _U.cmp(chad.x,bound) < 0 ? $Graphics$Collage.toForm(A3($Graphics$Element.image,79,71,sourcer("jasper-sign.png"))) : nothing;
      var signImage = _U.cmp(chad.x,bound) < 0 ? nothing : $Graphics$Collage.toForm(A3($Graphics$Element.image,1254,262,sourcer("birthday-sign.png")));
      var skinColor = A3($Color.rgb,252,162,132);
      var _p4 = {ctor: "_Tuple2",_0: $Basics.toFloat(_p6),_1: $Basics.toFloat(_p5)};
      var w = _p4._0;
      var h = _p4._1;
      return A3($Graphics$Collage.collage,
      _p6,
      _p5,
      _U.list([bluer(A2($Graphics$Collage.rect,w,h))
              ,A2($Graphics$Collage.move,{ctor: "_Tuple2",_0: chad.x,_1: 80 - h / 2},chadImage)
              ,A2($Graphics$Collage.move,{ctor: "_Tuple2",_0: chad.x + 10,_1: 200 - h / 2},chadSign)
              ,A2($Graphics$Collage.move,{ctor: "_Tuple2",_0: 300,_1: 80 - h / 2},jasperImage)
              ,A2($Graphics$Collage.move,{ctor: "_Tuple2",_0: 295,_1: 200 - h / 2},jasperSign)
              ,A2($Graphics$Collage.move,{ctor: "_Tuple2",_0: -20,_1: 500 - h / 2},signImage)]));
   });
   var chad = {x: -400,vx: 0};
   var main = A3($Signal.map2,view,$Window.dimensions,A3($Signal.foldp,update,chad,input));
   var Keys = F2(function (a,b) {    return {x: a,y: b};});
   var Model = F2(function (a,b) {    return {x: a,vx: b};});
